...
def load_resources():
    """
    Load heavy ML resources ONLY when an API call comes.
    This prevents Render port timeout.
    """
    global embedding_model, faiss_index, meta, resources_loaded
    ...
    # Paths (these must exist later, but NOT at startup)
    INDEX_PATH = "models/faiss.index"
    META_PATH = "models/meta.jsonl"
...
def chat_endpoint(request: ChatRequest):
    ...
    # TEMP response (safe for deployment)
    return {
        "query": request.message,
        "answer": "Backend is live and working 🎉",
        "retrieved_snippets": []
    }